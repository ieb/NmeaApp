...
(function () {
  ...
  */
  ...
  var layout = new _eink.FlowLayout(3, 2);
  layout.setPageTitle("Engine");
  layout.append(_eink.EInkTextBox.number("1.engine.[instance==0].speed", "engine", "rpm", 0));
  layout.append(_eink.EInkTextBox.number("1.engine.[instance==0].altenatorVoltage", "alternator", "V"));
  layout.append(_eink.EInkTextBox.number("1.engine.[instance==0].hours", "hours", "h", 2, 1 / 3600));
  layout.append(_eink.EInkTextBox.number("1.engine.[instance==0].coolantTemp", "coolant", "C", 1, 1, -273.15));
  layout.append(new EInkEngineStatus("1.engine.[instance==0]"));
  layout.append(_eink.EInkTextBox.number("1.fluidLevel.[instance==0,fluidType==0,debug].level", "fuel", "%", 0)); // should be fuel
  // No need to call new page as its full already.
  //layout.newPage();
  layout.setPageTitle("Batteries");
  layout.append(_eink.EInkTextBox.number("1.battery.[instance==0].voltage", "engine", "V"));
  layout.append(_eink.EInkTextBox.number("1.battery.[instance==1].voltage", "service", "V"));
  layout.append(_eink.EInkTextBox.number("1.engine.[instance==0].altenatorVoltage", "alternator", "V"));
  layout.append(_eink.EInkTextBox.number("1.battery.[instance==2].voltage", "engine controls", "V"));
  layout.newPage();
  layout.setPageTitle("Environment");
  layout.append(_eink.EInkTextBox.number("4.pressure.0.actual", "n2k", "mbar", 1, 0.01, 0));
  layout.append(_eink.EInkTextBox.number("4.pressure.0.actual", "n2k", "mbar", 1, 0.01, 0));
  layout.append(_eink.EInkTextBox.number("5.temperatureExt.0.actual", "n2k", "C", 1, 1, -273.15));
  layout.append(_eink.EInkTextBox.number("4.humidity.0.actual", "n2k", "%RH", 1));
  layout.newPage();
  layout.setPageTitle("Temperature");
  layout.append(_eink.EInkTextBox.number("6.temperatures.0.temperature", "Engine Room", "C", 2));
  layout.append(_eink.EInkTextBox.number("6.temperatures.1.temperature", "Exhaust", "C", 2));
  layout.append(_eink.EInkTextBox.number("6.temperatures.2.temperature", "Battery S", "C", 2));
  layout.append(_eink.EInkTextBox.number("6.temperatures.3.temperature", "Alternator", "C", 2));
  layout.newPage();
  layout.setPageTitle("Voltages");
  layout.append(_eink.EInkTextBox.number("8.battery.0.voltage", "service", "V", 2));
  layout.append(_eink.EInkTextBox.number("8.voltages.0.current", "service", "A", 1));
  layout.append(_eink.EInkTextBox.number("8.voltages.1.voltage", "engine", "V", 2));
  layout.append(_eink.EInkTextBox.number("8.voltages.1.current", "engine", "A", 1));
  layout.newPage();
  layout.setPageTitle("Performance");
  layout.append(_eink.EInkTextBox.number("performance.polarSpeed", 'polar stw', "Kn"));
  layout.append(_eink.EInkTextBox.number("performance.vmg", 'polar vmg', "Kn"));
  layout.append(_eink.EInkTextBox.number("performance.polarVmg", "best polar vmg", "Kn"));
  layout.append(_eink.EInkTextBox.number("performance.targetStw", "target stw", "Kn"));
  ...
  */
  ...
  var drawingContext = new _eink.EInkDrawingContext(drawingOptions);
  //        console.log("Using DataServer", window.DataServerAddress);

  var updater = new _eink.EInkUpdater({
    url: "".concat(window.DataServerAddress, "/api/data/all.json"),
  ...
  var root = _client["default"].createRoot(document.getElementById("appui"));
  root.render(dom("h1", null, "Hello World"));
})();