...
class NMEALayout extends React.Component {
    ...
    renderMenu(l) {
        ...
        const menuButton = this.state.showMenu?"\u2630":"\u2630"
        return (
            <div className={menuClass} >
                <button onClick={this.onMenuChange} className={indicatorClass} >{menuButton}</button>
                <PortControl bgPage={this.props.bgPage} />
                {l.layout.pages.map((page) => <button key={page.id} className={`pageButton ${(page.id == l.layout.pageId)?"activePage":""}`} onClick={this.onPageChange} value={page.id} >{page.name}</button>)}
    ...
    renderEditOverlay() {
        if ( this.props.editing ) {
            ...
            return (
                ...
                <select onChange={this.changeField} value={this.field} >
